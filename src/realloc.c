...
inline int is_tiny(struct t_block *tiny, void *ptr)
{
    return (tiny < ptr && ptr < (tiny + PREALOC_SIZE_TINY)) ? (1) : (0);
}

inline int is_small(struct t_block *small, void *ptr)
{
    return (small < ptr && ptr < (small + PREALOC_SIZE_TINY)) ? (1) : (0);
}
